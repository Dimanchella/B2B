...
Функция СформироватьДанныеГруппыНоменклатуры(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	Данные.Вставить("title", 	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Наименование));
	Данные.Вставить("parent",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Родитель));	
...
Функция СформироватьДанныеНоменклатуры(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	Данные = Новый Соответствие;
	Данные.Вставить("full_name", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Наименование));
	Данные.Вставить("type_of_product",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.ВидНоменклатуры));
	Данные.Вставить("group", 			web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Родитель));
...
Функция СформироватьДанныеИзображения(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	ТелоЗапроса.Добавить(web_Обмен.MFD_ЗаполнитьПараметр("id", XMLСтрока(СсылкаНаОбъект)));
	ТелоЗапроса.Добавить(web_Обмен.MFD_ЗаполнитьПараметр("product", XMLСтрока(СсылкаНаОбъект.ВладелецФайла)));
...
Функция ОтправитьИзображение(СсылкаНаОбъект, ПолнаяВыгрузка) Экспорт
	...
	web_ОбщегоНазначенияПовтИсп.Трассировка(СсылкаНаОбъект.ВладелецФайла);

	Возврат web_Обмен.ОтправитьMultipartFromData(
		"images", 
		СсылкаНаОбъект, 
...
Функция СформироватьДанныеПользователя(СсылкаНаОбъект, Пароль = Неопределено, ПолнаяВыгрузка)
	...
	Запрос.УстановитьПараметр("Вид", web_ОбщегоНазначенияПовтИсп.ПолучитьНастройку_ВидКИEmail());
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаОбъект);
	...
	Если Выборка.ИНН = Неопределено Тогда
		...
		Возврат Неопределено;
	КонецЕсли;
	Данные.Вставить("username", 	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.ИНН));
	...
	Если Выборка.Представление = Неопределено Тогда
		...
		Возврат Неопределено;
	КонецЕсли;
	Данные.Вставить("email", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Представление));
	
	Данные.Вставить("contractor",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(СсылкаНаОбъект));
	Данные.Вставить("full_name",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.НаименованиеПолное));
	
	Если ЗначениеЗаполнено(Пароль) Тогда
		Данные.Вставить("password",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Пароль));
	КонецЕсли;
...
Функция СформироватьДанныеВидаНоменклатуры(СсылкаНаОбъект, ПолнаяВыгрузка)
	Данные = Новый Соответствие;
	Данные.Вставить("name",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(СсылкаНаОбъект.Наименование));	
	Возврат Данные;
КонецФункции
...
Функция СформироватьДанныеХарактеристикиНоменклатуры(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	Данные = Новый Соответствие;
	Данные.Вставить("name", web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Наименование));
	
	Если ЗначениеЗаполнено(Выборка.Владелец) Тогда
		Владелец = web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Владелец);
...
Функция СформироватьДанныеПартнера(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	Данные = Новый Соответствие;
	Данные.Вставить("name", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(СсылкаНаОбъект.Наименование));
	Данные.Вставить("full_name",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(СсылкаНаОбъект.НаименованиеПолное));
	Возврат Данные;
КонецФункции
...
Функция СформироватьДанныеКонтрагента(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	Данные = Новый Соответствие;
	Данные.Вставить("inn", 			web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.ИНН));
	Данные.Вставить("kpp", 			web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.КПП));
	Данные.Вставить("name", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Наименование));
	Данные.Вставить("full_name",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.НаименованиеПолное));
	Данные.Вставить("partner", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Партнер));
...
Функция СформироватьДанныеСоглашения(СсылкаНаОбъект, ПолнаяВыгрузка)
	...
	Данные = Новый Соответствие;
	Данные.Вставить("name", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Наименование));
	Данные.Вставить("number", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Номер));
	Данные.Вставить("date", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Дата));
	Данные.Вставить("partner", 		web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Партнер));
	Данные.Вставить("contractor", 	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Контрагент));
	Данные.Вставить("organization",	web_ОбщегоНазначенияПовтИсп.ДобавитьЗначение(Выборка.Организация));
